...
{
    /// <summary>
    /// Plays SFX when a city is captured, using the common UI asset provider pattern.
    /// Place one in the world UI; it auto-discovers cities and IUiAssetProvider.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class CityCaptureSfxListener : MonoBehaviour
    {
        [Header("Assets")]
        ...
        [SerializeField, Tooltip("Addressables key for the city capture SFX.")]
        private string _sfxKey = "Audio/SFX/World/Capture-city";
        ...
        {
            var cities = FindObjectsOfType<CityAuthoring>(true);
            for (int i = 0; i < cities.Length; i++)
            {
                var c = cities[i];
                c.Claimed -= OnCityClaimed;
                c.Claimed += OnCityClaimed;
            }
        }

        private void UnsubscribeCities()
        {
            var cities = FindObjectsOfType<CityAuthoring>(true);
            for (int i = 0; i < cities.Length; i++)
            {
        ...
        private void OnCityClaimed(CityAuthoring city)
        {
