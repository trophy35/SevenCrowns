...
{
    /// <summary>
    /// Scene authoring component for a farm entry placed on the world map.
    /// Registers into FarmNodeService and can spawn a flag when claimed.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class FarmAuthoring : MonoBehaviour
    {
        private static readonly Dictionary<string, FarmAuthoring> s_ByNodeId = new(StringComparer.Ordinal);

        [Header("Identity")]
        [Tooltip("Stable identifier for this farm. Auto-generated when empty.")]
        ...
        [Header("Production")]
        [Tooltip("Population yield per in-game week produced by this farm.")]
        [SerializeField, Min(0)] private int _weeklyPopulationYield = 20;

        private FarmNodeService _service;
        ...
        public static bool TryGetNode(string nodeId, out FarmAuthoring node)
        {
            ...
            var candidates = FindObjectsOfType<FarmAuthoring>(true);
        ...
        private void ResolveService()
        {
            ...
            {
                if (behaviours[i] is FarmNodeService service)
                {
                    _service = service;
                    break;
                }
            ...
            {
                Debug.LogWarning($"[FarmAuthoring] No {nameof(FarmNodeService)} found in scene.", this);
            }
        }
        ...
        private void ResolveEntryCoord()
        {
            ...
            if (_snapToGrid && _grid != null)
            {
                ...
                {
#if UNITY_EDITOR
                    Debug.LogWarning($"[FarmAuthoring] Grid reference differs from provider's GroundGrid. Using provider grid for coord mapping.", this);
#endif
...
#if UNITY_EDITOR
                        Debug.LogWarning($"[FarmAuthoring] Farm '{name}' position is outside provider bounds; clamping entry to {localClamped}. Consider increasing bounds padding or moving the farm.", this);
#endif
                    }
                }
                else
                {
                    Debug.LogWarning($"[FarmAuthoring] No TilemapTileDataProvider assigned; cannot compute entry coord.", this);
                }
            }
            ...
            if (ctm != null)
            {
                ...
                if (prov != null)
                {
                    ...
                    if (_logMissingService)
                        Debug.Log("[FarmAuthoring] Auto-bound TilemapTileDataProvider from ClickToMoveController.", this);
                }
            }
            if (_tileDataProvider == null)
            {
                ...
                if (prov != null)
                {
                    _tileDataProvider = prov;
                    if (_logMissingService)
                        Debug.Log("[FarmAuthoring] Auto-bound first TilemapTileDataProvider in scene.", this);
                }
            }
        }
        ...
        private void RegisterNode()
        {
            ...
            var descriptor = new FarmNodeDescriptor(
                _nodeId,
                transform.position,
                _entryCoord,
                _isOwned,
                _ownerId,
                Mathf.Max(0, _weeklyPopulationYield));
            _service.RegisterOrUpdate(descriptor);
            _isRegistered = true;
            if (_logRegistration)
            {
                Debug.Log($"[FarmAuthoring] Registered farm id='{_nodeId}' entry={(_entryCoord.HasValue ? _entryCoord.Value.ToString() : "<null>")} provider='{(_tileDataProvider!=null?_tileDataProvider.name:"<null>")}' grid='{(_grid!=null?_grid.name:"<null>")}'.", this);
            }
        }
        ...
        private System.Collections.IEnumerator RegisterWhenReady()
        {
            ...
            if (_logMissingService)
            {
                Debug.LogWarning("[FarmAuthoring] Timed out waiting for TilemapTileDataProvider bake; farm entry may be invalid.", this);
            }
        }

        /// <summary>
        /// Claims this farm for the current player and spawns/updates a flag sprite at the configured offset.
        /// </summary>
        ...
        private void SpawnOrUpdateFlag(Color? overrideColor)
        {
            ...
            {
                Debug.LogWarning("[FarmAuthoring] Entry coord not resolved; cannot place flag sprite.", this);
                return;
            }
            ...
            if (_flagInstance == null)
            {
                ...
                if (_flagPrefab != null)
                {
                    ...
                    _flagInstance.name = string.IsNullOrEmpty(_nodeId) ? "FarmFlag" : $"FarmFlag_{_nodeId}";
                }
                else
                {
                    _flagInstance = new GameObject(string.IsNullOrEmpty(_nodeId) ? "FarmFlag" : $"FarmFlag_{_nodeId}");
        ...
        public event Action<FarmAuthoring> Claimed;
    }
}
