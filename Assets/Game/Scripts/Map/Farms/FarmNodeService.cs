...
{
    /// <summary>
    /// Tracks farm nodes present in the active world map and exposes read-only access to other systems.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class FarmNodeService : MonoBehaviour, IFarmNodeProvider
    {
        ...
        private readonly List<FarmNodeDescriptor> _nodes = new();
        ...
        public event Action<FarmNodeDescriptor> NodeRegistered;
        public event Action<FarmNodeDescriptor> NodeUpdated;
        public event Action<string> NodeUnregistered;

        public IReadOnlyList<FarmNodeDescriptor> Nodes => _nodes;

        public bool RegisterOrUpdate(FarmNodeDescriptor descriptor)
        {
            if (!descriptor.IsValid)
            {
                Debug.LogWarning($"[FarmNodeService] Attempted to register invalid node descriptor (id='{descriptor.NodeId}').", this);
                return false;
            }
        ...
        public bool TryGetById(string nodeId, out FarmNodeDescriptor descriptor)
        {
            if (!string.IsNullOrWhiteSpace(nodeId) && _indexById.TryGetValue(nodeId, out int index))
            {
        ...
        public bool TryGetByCoord(GridCoord coord, out FarmNodeDescriptor descriptor)
        {
        ...
        private void UpdateCoordMapping(FarmNodeDescriptor previous, FarmNodeDescriptor current)
        {
            RemoveCoordMapping(previous.NodeId, previous);
            ApplyCoordMapping(current);
        }

        private void ApplyCoordMapping(FarmNodeDescriptor descriptor)
        {
            ...
            if (_nodeIdByCoord.TryGetValue(coord, out var existingId) && !string.Equals(existingId, descriptor.NodeId, StringComparison.Ordinal))
            {
                ...
                {
                    Debug.LogWarning($"[FarmNodeService] Entry coordinate {coord} is already mapped to node '{existingId}'. Overwriting with '{descriptor.NodeId}'.", this);
                }
            }
        ...
        private void RemoveCoordMapping(string nodeId, FarmNodeDescriptor descriptor)
        {
            if (descriptor.HasEntryCoord)
            {
                var coord = descriptor.EntryCoord.Value;
